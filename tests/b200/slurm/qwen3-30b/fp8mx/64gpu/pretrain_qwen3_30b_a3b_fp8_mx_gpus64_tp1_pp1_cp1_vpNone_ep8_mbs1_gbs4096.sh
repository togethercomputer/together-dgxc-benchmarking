...
bash -c 'CPUSET=
    ...
    numactl --physcpubind=${CPUSET} --membind=${MEMNODE} python /mnt/vast/exemplar/llmb/workloads/pretrain_qwen3/Megatron-Bridge/scripts/performance/run_script.py --container_image /mnt/vast/exemplar/llmb/images/nvidia+nemo+25.11.01.sqsh --compute_dtype fp8_mx --gpu b200 --num_gpus 64 --gpus_per_node 8 --model_name qwen3 --model_size 30b_a3b --custom_mounts /mnt/vast/exemplar/llmb/.cache/huggingface -tp 1 -pp 1 -cp 1 -ep 8 -et 1 -gb 4096 -mb 2 --cuda_graph_impl=transformer_engine --cuda_graph_scope=moe_router,moe_preprocess --account root --partition batch --log_dir /mnt/vast/exemplar/llmb/workloads/pretrain_qwen3 --time_limit 00:40:00 --max_steps 50 --hf_token HF_TOKEN_REDACTED train.manual_gc=true train.manual_gc_interval=100'