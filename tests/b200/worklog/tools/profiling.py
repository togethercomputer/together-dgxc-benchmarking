...
def _chrome_trace_handler(output_dir):
    """Custom trace handler that writes compressed Chrome traces with rank info."""
    def handler(prof):
        hostname = socket.gethostname()
        rank = os.environ.get('RANK', os.environ.get('SLURM_PROCID', str(os.getpid())))
        filename = f"{hostname}_rank{rank}.pt.trace.json.gz"
...
def initialize_pytorch_profiler(
    ...
    Args:
        config: Profiling configuration
        tensorboard_dir: Directory for trace outputs
    ...
    prof = torch.profiler.profile(
        ...
        on_trace_ready=_chrome_trace_handler(tensorboard_dir),
